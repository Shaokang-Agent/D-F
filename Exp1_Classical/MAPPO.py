...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/MAPPO-norm/" + str(round_num))
    Pi = []
    V = []
    ...
    for i_episode in tqdm(range(n_episode)):
        ...
        while steps < max_steps and not done:
            ...
            if steps % T == 0:
                ...
                for i in range(n_agent):
                    Pi[i].update(ep_obss[i], ep_actions[i], all_ep_advantages_saved[i])
        ...
        print(config['agent']['env_name'], "MAPPO-norm", round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/MAPPO-norm.npy", reward_step)
