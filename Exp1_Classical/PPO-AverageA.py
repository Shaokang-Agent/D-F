...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/PPO-AverageA/" + str(round_num))
    gPi = []
    gV = []
    ...
    for i_episode in tqdm(range(n_episode)):
        ...
        while steps < max_steps and not done:
            ...
            if steps % T == 0:
                ...
                for i in range(n_agent):
                    gPi[i].update(ep_states[i], ep_actions[i], all_ep_advantages)
        ...
        print(config['agent']['env_name'], "PPO-AverageA", round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/PPO-AverageA.npy", reward_step)
