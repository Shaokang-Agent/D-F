...
import copy
from common.utils import eligibility_traces, default_config, make_env, str2bool, get_more_obs_com, discount_rewards
...
alpha_fairness = float(config['agent']['alpha_fairness'])
...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/ALF/" + str(round_num))
    gPi = []
    Pi = []
    gV = []
    V = []
    ...
    for i_episode in tqdm(range(n_episode)):
        ...
        print(config['agent']['env_name'], "SOTO-ALF", round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/ALF.npy", reward_step)
