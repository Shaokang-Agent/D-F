...
for round_num in range(round):
    writter = SummaryWriter("./" + config['agent']['env_name'] + "/runs/PPO-FEN/" + str(round_num))
    gPi = []
    gV = []
    ...
    for i_episode in tqdm(range(n_episode)):
        ...
        print(config['agent']['env_name'], "PPO-FEN", round_num, i_episode)
        print(su, su.sum())
        writter.add_scalar("Total Rewards", su.sum(), i_episode)
        reward_step[round_num, i_episode] = su
        np.save("./" + config['agent']['env_name'] + "/PPO-FEN.npy", reward_step)
